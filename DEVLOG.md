...
### 2026-09-19 — Preserve named speakers and their project relationships

- A user-reviewed meeting speaker reached retention correctly, but extraction
  created project identities without the person and attached some of the speaker's
  personal experiences to canonical You. Source labels and provenance were intact.
  Clarify the existing retention prompt: supplied speaker names are identity
  evidence; retain the person alongside their project when a statement concerns
  both; distinguish the account owner, a speaker's own involvement, and reporting
  another person's work. Clarify presentation of useful person/project context on
  both pages, using the existing shared evidence and linked-subject mechanism.
- No model stage, schema field, role metadata, page-admission heuristic, sampling
  setting, retry allowance, or API changes. The two production prompts grow from
  435 to 579 words combined. Add one end-to-end structural regression from reviewed
  Engram speaker assignment through capture, retention input, subject references,
  linked person/project pages, and source citations; it asserts one call per stage.
- Native evidence: `benchmark_runs/speaker-attribution-20260919/`. Before production
  integration, candidate-v1 direct calls using production schemas create the named
  builder and project, and correctly distinguish a reporter from a third-party
  builder. The ordinary-chat probe links the user's activity to You but fails
  validation three times because You is absent from the returned subjects list.
  Add one general sentence making this existing declaration requirement explicit.
  A single direct recheck with the final candidate succeeds, preserving You and the
  user's decision not to publish the notebook. No code reconstructs missing output.
- Freeze the recording's first 100 contiguous segments before probing, preserving
  speaker metadata. Run baseline and final production prompts through ordinary
  Build in separate fresh stores containing canonical You. Retention user payloads,
  schemas, model and sampling options match exactly. Baseline: 6 claims, 3 view
  items, two project pages and no person page; 2 requests, 35.03s wall time,
  21,128 input / 2,206 output tokens. Candidate: 7 claims, 5 view items, a Hari page
  linked to a project plus an additional topic page; 2 requests, 51.63s wall time,
  22,320 input / 3,459 output tokens. Both Builds complete without retries; neither
  assigns these excerpt claims to You. Attribution improves without adding calls,
  but this sample takes about 47% longer and generates more output.
- Source review still finds excessive sharing of project details onto the person
  page. Candidate retention incorrectly assigns curriculum design to agents where
  the speaker describes designing it himself; both versions overstate current
  automation. These are residual semantic errors, not grounds for another tuning
  loop here. This is one sample per prompt, with configured stochastic sampling,
  no prior conversations, and only the opening excerpt. It does not establish full
  meeting quality, cross-build identity consistency, or general performance gains.
- Stop at exactly 12 native generation requests including the three rejected
  outputs: 69,091 input / 8,249 output tokens, 137.73 server seconds, no transport
  failures. To stay within the cap, do not run chat presentation after the successful
  retention recheck or repeat the earlier positive/counterexample under the final
  declaration sentence. Saved requests, outputs, stores, prompts and `review.json`
  preserve these limits and the comparison for inspection.
- Validation: 39 focused Build, Engram, concurrency, view and lifecycle tests pass
  in 3.55s; Ruff and whitespace checks pass. Cancel a stalled sandboxed FastAPI test
  run and rerun the focused suite with host access under existing user authorization.
  The live store is inspected read-only and remains unchanged; completed extraction
  batches are not regenerated by these prompt changes. The separate long-recording
  token-overflow failure remains outside this update. No app service was started.

### 2026-09-19 — Generalize organization prompts and supply existing page links

- User asks to test simpler, general instructions before adopting them. Replace
  the person/project-specific recipe with guidance about useful subjects, who did
  what, relationships and duplication. Shorten attribution to speaker identity,
  relevant subject references and the distinction between speakers and people
  they describe. Retain citation, identity-review, user-edit and output-contract
  requirements. Combined prompt length falls from 579 to 503 words (13%).
- View refresh previously omitted stored sharing links from existing_items. Supply
  linked_subject_ids from each existing fact so the model can see the current page
  organization. This exposes existing state in the input; it adds no persisted
  field, output-schema requirement, model call, retry policy or admission heuristic.
  Extend the Engram-to-memory regression through a second Build: it reproduces the
  missing input before the fix, then confirms shared and unshared items, citations
  and page links survive without re-extracting the source.
- Native evidence: `benchmark_runs/general-memory-prompts-20260919/`. Test one
  candidate with the configured Gemma model and production schemas before editing
  production prompts. The named-builder case creates the person and project; the
  reporter case distinguishes the developer from the reporter; role=user remains
  associated with You. A direct refresh with the proposed sharing field preserves
  the neutral example's links. Ordinary chat still overstates an undecided
  publication choice as not for public distribution; the reporter example gains
  an incidental page and repeats some content. Do not tune around these misses.
- Fresh matched Builds use the same frozen 100 opening recording segments in
  isolated stores. Retention payloads, schemas, model and sampling options match.
  Longer baseline: 2 requests, 35.09s wall time, 21,170 input / 2,266 output tokens,
  4 claims and 4 view items displayed 8 times across pages. General candidate:
  2 requests, 36.40s wall time, 21,193 input / 2,348 output tokens, 4 claims and
  3 view items displayed 3 times. Candidate prose retains named attribution and
  eventual automation as a goal, although nuanced curriculum context is simplified.
- Both fresh Builds omit a person entity for the named speaker. The longer prompt
  produced it in the previous run, so the earlier success was not a reliable
  guarantee. The new candidate is not a demonstrated identity-recall fix. Reduced
  duplication is an observation from different end-to-end outputs, not an isolated
  causal test of presentation or evidence of general quality/performance gains.
- Use the final budgeted call to isolate presentation with available person context:
  make a read-only SQLite backup of the earlier isolated candidate store containing
  the person, project and shared items, then refresh through production ViewOrganizer.
  The general prompt receives existing links and preserves all five items' text,
  ownership, citations and sharing. Hari and project pages remain available. This
  also preserves preexisting summarization errors; it does not recover missing
  identity evidence or claim correctness from the original recording.
- Adopt the general candidate for useful, comparable behavior with fewer
  scenario-specific instructions, alongside the corrected sharing input. Stop at
  the fixed 12-request allowance: 63,054 input / 8,161 output tokens, 132.25 server
  seconds; no transport failures or structured-output retries. Saved requests,
  prompts, inputs, stores and review.json record the evidence and limits. Samples
  are small and familiar, with one stochastic run per variant and no full meeting
  replay. No additional prompt variants or automatic semantic repairs are added.
- Validation: all 24 affected Build, view, correction and transaction tests pass;
