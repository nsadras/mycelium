...
|---|---|---|
| A. Benchmark integrity | Independent completion statuses; default QA block on incomplete LoCoMo encoding; invocation/config/code/model provenance; typed citation coverage; optional reference judgments; daily-driver per-probe timing/evidence/traces, strict judgment IDs and uniquely recorded native attempts | Semantic scorer21 neutral trials and reporting/resume smoke passed; daily judge9/9. Complete longitudinal source/wiki audit and comparable full quality/cost runs remain. Interrupted invocation duration is unknown; reference judging is not a source-grounding audit. |
...
| P. Cleanup | Public retrieval errors and config/network docs; retired work-unit/selection fields removed; database cleanup now releases resources safely on collector threads while ordinary operations remain thread-affine | Continue cleanup of unused contracts and prompts where exercised by remaining mechanisms. |
