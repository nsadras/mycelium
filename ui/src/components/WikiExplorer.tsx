...
export default function WikiExplorer({ onInspectReview }: { onInspectReview?: (proposalId: string) => void }) {
  ...
  const detailCount = (pageData?.sections ?? []).flatMap(section => section.items)
    .filter(item => item.kind === 'fact' && item.prominence === 'detail').length;
  const visibleSections = (pageData?.sections ?? []).map(section => ({ ...section,
    items: section.items.filter(item => showDetails || item.kind !== 'fact' || item.prominence !== 'detail'),
...
function FactEvidence({ item, entities, heading, currentEntityId, onMoved }: { item: WikiFactItem; entities: EntityRecord[]; heading: string; currentEntityId: string; onMoved: (slug?: string | null) => Promise<void> }) {
  ...
  const changeOwner = (value: string) => setOwner(value);
  ...
  const ownerChoices = entities.filter((entity) => entity.status === 'active');
  return <div className="mt-4 border-t border-slate-100 pt-3 text-xs text-slate-500"><div className="mb-2 font-semibold text-slate-700">Consolidated fact</div><ClaimCorrectionEditor claimIds={item.claim_ids} onSaved={() => onMoved()} />{item.claim_ids.length > 1 && <button onClick={split} className="mt-2 rounded bg-white px-3 py-2 font-semibold text-slate-600 ring-1 ring-slate-200">Split claims</button>}<div className="mt-2 text-[11px]">{item.synthesis_reason} · confidence {item.synthesis_confidence.toFixed(2)}</div>{item.temporal_evidence.length > 0 && <><div className="mb-2 mt-3 font-semibold text-slate-700">Time evidence</div>{item.temporal_evidence.map((temporal, index) => <div key={`${temporal.expression}-${index}`} className="mb-1 rounded bg-slate-50 p-2"><span className="font-semibold">Source wording:</span> {temporal.expression}{temporal.start && <span> · normalized: {temporal.start}{temporal.end && temporal.end !== temporal.start ? `–${temporal.end}` : ''}</span>}{temporal.role && <span> · {temporal.role.replaceAll('_', ' ')} for {temporal.target}</span>}{(temporal.status === 'unresolved' || temporal.status === 'recurring') && <span> · {temporal.status}</span>}{temporal.reference_reason && <div className="mt-1">{temporal.reference_reason}{temporal.anchor && <span> · reference: {temporal.anchor}</span>}</div>}</div>)}</>}<div className="mb-2 mt-3 font-semibold text-slate-700">Canonical claims</div>{item.claim_ids.map((id) => <div key={id} className="break-all font-mono">{id}</div>)}<div className="mt-2">View owner: {entities.find((entity) => entity.entity_id === canonicalOwnerId)?.title ?? canonicalOwnerId}</div><div className="mb-2 mt-3 font-semibold text-slate-700">Exact source evidence</div>{item.sources.map((source, index) => { const artifact = sourceArtifacts[source.source_id]; const segments = artifact?.segments.filter((segment) => source.segment_ids.includes(segment.segment_id)) ?? []; return <div key={`${source.source_id}-${index}`} className="mb-2 rounded bg-slate-50 p-2"><div className="font-mono">{source.source_id}</div>{segments.length ? segments.map((segment) => <div key={segment.segment_id} className="mt-2 border-l-2 border-indigo-200 pl-2"><div className="mb-1 font-mono text-[10px] text-slate-400">{segment.segment_id}{segment.speaker ? ` · ${segment.speaker}` : ''}</div><div className="whitespace-pre-wrap text-slate-700">{segment.content}</div></div>) : <div className="mt-1 break-all">{source.segment_ids.join(', ')}</div>}</div>; })}{item.links.length > 0 && <div className="mt-2">Linked entities: {item.links.map((link) => link.title).join(', ')}</div>}<div className="mt-4 grid gap-2 rounded-lg border border-slate-200 p-3 md:grid-cols-[1fr_1fr_auto]"><select value={owner} onChange={(event) => changeOwner(event.target.value)} className="rounded border-slate-200 text-xs">{ownerChoices.map((entity) => <option key={entity.entity_id} value={entity.entity_id}>{entity.title}</option>)}</select><input aria-label="Heading" value={section} onChange={(event) => setSection(event.target.value)} className="rounded border-slate-200 text-xs" /><button onClick={move} className="rounded bg-indigo-600 px-3 py-2 font-semibold text-white">Move</button></div></div>;
}
