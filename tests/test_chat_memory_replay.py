...
async def assert_tool_pages(memory, fixture, tool_source_ids, tmp_path):
    ...
    tool_claim_ids = {
        c.claim_id
        for c in memory.artifacts.list_claims(status="active")
    ...
    for page in memory.wiki.list():
        ...
        if page.entity_id == "you":
            assert not grounded, (
                "Restaurant search results became personal facts about You"
            )
        elif grounded:
            pages.append(
                {
                    ...
                    "type": page.page_type,
                    "facts": [
                        {"text": f["text"], "claim_ids": f["claim_ids"]}
                        for f in grounded
                    ],
                }
            )
    expected = {
        f"E{i:03d}": e for i, e in enumerate(fixture["expected_tool_entities"], 1)
    }
    schema = create_model(
        "ExpectedToolPages",
    ...
    verdict = schema.model_validate(
        await memory.llm.call_structured(
            ...
            json.dumps({"expected": expected, "pages": pages}),
            schema,
            num_predict=2048,
        )
    ).model_dump()
    (tmp_path / "tool_page_review.json").write_text(
        json.dumps({"pages": pages, "verdict": verdict}, indent=2)
    )
    by_id = {p["entity_id"]: p for p in pages}
    for key, match in verdict.items():
        assert match["supported"] and match["entity_id"] in by_id, (
            expected[key],
            match,
        )
    ...
    for left, right in fixture["expected_shared_tool_page_pairs"]:
        left_claims = {
            c
            for f in by_id[verdict[left]["entity_id"]]["facts"]
            for c in f["claim_ids"]
        }
        right_claims = {
            c
            for f in by_id[verdict[right]["entity_id"]]["facts"]
            for c in f["claim_ids"]
        }
        assert left_claims & right_claims, (
            "Related pages did not share a canonical statement"
        )
...
@pytest.mark.asyncio
async def test_chat_history_rebuilds_user_and_tool_pages_and_recalls_cooking(
    ...
    configure_sessions(monkeypatch, runtime, store / "sessions_meta.json")
    ...
    expected_tools = [
        (c["session_id"], index // 2 + 1, message, event)
        for c in fixture["conversations"]
    ...
    for sid, turn_count, message, event in expected_tools:
        matches = [
            ...
            if s.metadata["chat_session_id"] == sid
    ...
    source_ids_by_session = {
        c["session_id"]: {
            s.source_id for s in sources if s.session_id == c["session_id"]
        }
        for c in fixture["conversations"]
    }
    ...
    for conversation in fixture["conversations"]:
        ...
        assert [
            ...
            {k: m[k] for k in ("role", "content", "timestamp")}
            for m in conversation["messages"]
        ]

    print(
        f"Captured {len(chat_sources)} chat turns and {len(tool_sources)} frozen tool observations; building memory.",
        flush=True,
    )
    ...
    assert all(
        e.extraction_status == "complete" for e in memory.artifacts.list_episodes()
    )

    pages = memory.wiki.list()
    (tmp_path / "pages.json").write_text(
        json.dumps([asdict(p) for p in pages], indent=2, default=str)
    )
    for page in pages:
        occurrences = Counter(
            claim_id
    ...
    user_pages = [
        p for p in pages if p.page_type == "you" or p.title.casefold() == "you"
    ]
    ...
    facts = [
        item
        for section in memory.wiki.get("you").sections
        for item in section["items"]
        if item["kind"] == "fact"
    ]
    owned_source_ids = {
    ...
    response = await sessions.chat(
        recall["session_id"],
        sessions.ChatRequest(message=recall["message"]),
    )
    ...
    print(
        "PASS: one populated You page, tool-grounded restaurant/person pages, and cooking-source recall.",
        flush=True,
    )
