...
| J. Identity scaling | Durable schema-validated identity/routing reuse checks full requests/settings/model digests; explicit reviewed occurrence IDs bind only their particular identity, support multiple subjects in one claim, and preserve independent reviews | Reviewed occurrence contract passed12 direct and12 native routing trials; schema3 rejects older stores without migration. Bounded semantic candidate index (24) and avoidance of unchanged impossible retries remain; full longitudinal gates still apply. |
