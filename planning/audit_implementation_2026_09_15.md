...
| D. Workspace correctness | Equal-revision canonical claims/citations merge; newer interpretations replace older; canonical refresh between tools; whole-record fit includes workspace envelope; monotonic revisions with eight retained operations and budgeted diagnostics on success/failure | Source revisions and deleted-source races; browser inspection tests. Repeated rejection tests preserve evidence within the configured budget. |
...
| G. UI races | Cancel obsolete wiki fetches; chat generation guards; preserve failed drafts and remove failed optimistic message; ignore delayed reply/status updates; correction editor rejects obsolete loads and retains retryable date reviews | Five correction-editor component tests now pass. Remaining: request-order tests for wiki/chat/Engram; Engram mutation responses; hide stale transcript while loading another session; draft restoration if history fetch fails. |
