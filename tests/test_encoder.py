...
@pytest.mark.asyncio
async def test_capture_session_skips_empty_transcript(
...
async def test_ingest_source_returns_an_explicit_empty_result(
    ...
    result = await encoder.ingest_source(
        SourceInput(transcript="   ", session_id="ses-123")
    )
...
async def test_ingest_source_rejects_explicitly_empty_segments_for_nonempty_input(
    ...
    encoder.log_store = LogStore(encoder.artifacts.root.parent / "logs")
    with pytest.raises(
        ValueError,
        match="non-empty source transcript must produce at least one segment",
    ):
        await encoder.ingest_source(
            SourceInput(
                transcript="USER: Keep this memory.",
                session_id="ses-123",
                segments=(),
    ...
    assert encoder.artifacts.list_ingestion_operations() == []
    assert encoder.log_store.list_entries(days=None) == []
