...
| J. Identity scaling | Source-first discovery, separate exact human-review assignment, typed semantic candidates bounded to24 per subject, chunked changed-document embeddings and durable model/query reuse; source attribution sees resolved subjects without page/review metadata; page presentation receives only eligible described subjects; provisional identity matches retain pending review; explicit agent-chat user participants and a separately required declared-user output | Revised review sequence passed12 direct and12 native trials; larger-registry native12/12; six continuity scenarios across three builds retained identity/reviews and made zero generations on every third build. Required-user direct9/9 and native identity separation passed; one downstream placement counterexample remains. Full longitudinal identity/coverage and growth-cost gates still apply. |
