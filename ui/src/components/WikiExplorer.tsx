...
function FactEvidence({ item, entities, ontology, currentEntityId, onMoved }: { item: WikiFactItem; entities: EntityRecord[]; ontology: MemoryOntology; currentEntityId: string; onMoved: (slug?: string | null) => Promise<void> }) {
  ...
  const [editingClaimId, setEditingClaimId] = useState(item.claim_ids[0] ?? '');
  const [saving, setSaving] = useState(false);
  const [correctionError, setCorrectionError] = useState<string | null>(null);
  useEffect(() => {
    Promise.all(item.claim_ids.map(async id => [id, (await api.get(`/memory/artifacts/claims/${encodeURIComponent(id)}`)).data.text] as const))
      .then(entries => { const texts = Object.fromEntries(entries); setClaimTexts(texts); setEditingClaimId(item.claim_ids[0]); setFactText(texts[item.claim_ids[0]] ?? ''); })
      .catch(() => setCorrectionError('Could not load the statements to correct.'));
  }, [item]);
  ...
  const saveText = async () => {
    setSaving(true); setCorrectionError(null);
    try { await api.post(`/memory/claims/${encodeURIComponent(editingClaimId)}/correct`, { text: factText }); await onMoved(); }
    catch { setCorrectionError('The correction was not completed. You can retry safely.'); }
