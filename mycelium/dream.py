...
class ConsolidationProcess:
    ...
    async def run(
        ...
        *,
        ...
        prepared = self.prepare(
            include_deferred=include_deferred, source_ids=source_ids
        )
        ...
        newly_materialized = [
            ...
            and (
                entity.entity_id not in previous_entities
                or previous_entities[entity.entity_id].materialization_state
                != "materialized"
            )
        ]
        ...
        if routing is not None:
            ...
            for failure in routing.failures:
                ...
                self.policy.set_decision(
                    decisions, failure.claim_id, "routing_failed", failure.reason
                )
            successful_routes = [
                route
        ...
        for failure in fact_result.failures:
            ...
            for route in successful_routes:
                ...
                affected = (
                    route.claim_id in failure.claim_ids
                    if failure.partial
        ...
        successful_routes = [
            ...
            if route.claim_id in placement_updates
            else route
            for route in successful_routes
        ]
        ...
        routed_entities = {
            ...
            for entity in [*self.artifacts.list_entities(), *retained_new_entities]
        }
        for route in successful_routes:
            if route.placed:
        ...
        completed_source_ids = [
            ...
            if entry.entry_id not in failed_source_ids
        ]
        pending_source_ids = [
            entry.entry_id
            for entry in raw_entries
            if entry.entry_id in failed_source_ids
        ]
        report = DreamReport(
    ...
    def commit(self, inputs: DreamCommitInput) -> None:
        ...
        commit = self.committer.prepare(
            ...
            entity_decisions=inputs.routing.entity_decisions
            if inputs.routing is not None
            else [],
            entity_references=inputs.routing.entity_references
            if inputs.routing is not None
            else [],
    ...
    def prepare(
        self, *, include_deferred: bool, source_ids: set[str] | None = None
    ) -> DreamPreparation:
        started_at = datetime.now().astimezone().isoformat()
        run_id = (
            f"dream-{datetime.now().strftime('%Y%m%dT%H%M%S')}-{uuid.uuid4().hex[:8]}"
        )
        ...
        if source_ids is not None:
            ...
            raw_entries = [
                entry for entry in raw_entries if entry.entry_id in snapshot_log_ids
            ]
        ...
        for entry in raw_entries:
            ...
            if episode is None or episode.extraction_status != "complete":
                failed_source_ids.add(entry.entry_id)
                failures.append(
                    {
                        "stage": "extraction",
                        "source_id": entry.entry_id,
                        "reason": episode.extraction_error
